...
def SetupConfigurationFileOnClient(resource_id,userid,ip_address_or_domain,userpw=None, install_swf_path_server = None, sshport=22):
    ...
    if config_file_path == None:
        ...
        config_parser   =   ConfiguratePaser(config_parser,resource_id,hostname,info_queue,userid,install_swf_path_server,sshport)
        WriteOutConfiguration(config_parser,config_file_path)
        ...
        if any( sec ==  resource_id for sec in list_sections):
            ...
            info_queue=GetQueueNamesOnPBSTORQUE(userid, ip_address_or_domain,userpw)
            config_parser=ConfiguratePaser(config_parser,resource_id,ip_address_or_domain,info_queue,userid,install_swf_path_server,sshport)
            WriteOutConfiguration(config_parser,config_file_path)
...
def CopySomaWF2Server(install_swf_path_server,userid,ip_address_or_domain,userpw='',sshport=22):
    ...
    install_swf_path_server_soma_workflow=os.path.join(install_swf_path_server,"soma")
    install_swf_path_server_drmaa=os.path.join(install_swf_path_server)
    
    std_out_lines=SSHExecCmd("mkdir -p '%s'"%(install_swf_path_server_drmaa),userid,ip_address_or_domain,userpw,sshport=sshport)
    print std_out_lines
    std_out_lines=SSHExecCmd("mkdir -p '%s'"%(install_swf_path_server_soma_workflow),userid,ip_address_or_domain,userpw,sshport=sshport)
    print std_out_lines
...
def CheckIfSomaWFonServer(install_swf_path_server,ResName,userid,ip_address_or_domain,userpw='',sshport=22):
    
    command= "ps -ef | grep 'python -m soma.workflow.start_database_server' | grep '%s' \
| grep -v grep | awk '{print $2}'"%(ResName)

    std_out_lines=SSHExecCmd(command,userid,ip_address_or_domain,userpw,wait_output=True,sshport=sshport)
...
def InstallSomaWF2Server(install_swf_path_server,ResName,userid,ip_address_or_domain,userpw='',sshport=22):
    ...
    SetupConfigurationFileOnClient(ResName, userid,ip_address_or_domain,userpw,install_swf_path_server,sshport)
...
def SetupSomaWF2Server(install_swf_path_server,ResName,userid,ip_address_or_domain,userpw='',sshport=22):
    ...
    if CheckIfSomaWFonServer(install_swf_path_server,ResName,userid,ip_address_or_domain,userpw,sshport) == False:
...
#    print "sshport="+repr(sshport)
    ...
    clean_script_server=install_swf_path_server+"/python/soma/workflow/clean_server.py"
