...
class JobExamples(object):
    ...
    def __init__(self, wf_ctrl, inpath, outpath,
                 python, transfer_timeout=-24, jobs_timeout=1):
        ...
        self.job1OutputFileModels = [
            self.inpath + "complete/outputModels/file11",
            self.inpath + "complete/outputModels/file12"]
        self.job2OutputFileModels = [
            self.inpath +
            "complete/outputModels/file2"]
        self.job3OutputFileModels = [
            self.inpath +
            "complete/outputModels/file3"]
        self.job4OutputFileModels = [
            self.inpath +
            "complete/outputModels/file4"]

        self.job1stdouterrModels = [
            self.inpath + "complete/outputModels/stdoutjob1",
            self.inpath + "complete/outputModels/stderrjob1"]
        self.job2stdouterrModels = [
            self.inpath + "complete/outputModels/stdoutjob2",
            self.inpath + "complete/outputModels/stderrjob2"]
        self.job3stdouterrModels = [
            self.inpath + "complete/outputModels/stdoutjob3",
            self.inpath + "complete/outputModels/stderrjob3"]
        self.job4stdouterrModels = [
            self.inpath + "complete/outputModels/stdoutjob4",
            self.inpath + "complete/outputModels/stderrjob4"]

        self.exceptionjobstdouterr = [
            self.inpath + "simple/outputModels/stdout_exception_job",
    ...
    def submitJob1(self, time=2):
        ...
        return (
            (job1_id, [self.file11_tr.engine_path, self.file12_tr.engine_path], None)
        )
    ...
    def mpiJobSubmission(self, node_num):
        ...
        job_id = self.wf_ctrl.submit_job(
            Job(command=[script, repr(node_num), bin_tr],
                referenced_input_files=[
                    script, bin_tr],
                join_stderrout=False,
                disposal_timeout=self.jobs_timeout,
...
class JobsTest(unittest.TestCase):
    ...
    def setUp(self):
        raise Exception(
            'JobTest is an abstract class. SetUp must be implemented in subclass')
    ...
    def test_wait2(self):
        ...
        if delta < timedelta(seconds=interval):
            ...
            self.failUnless(
                abs(delta - timedelta(seconds=interval)) < timedelta(seconds=1))
    ...
    def test_kill(self):
        ...
        self.failUnless(
            exitStatus == constants.USER_KILLED or exitStatus == constants.FINISHED_REGULARLY,
            'Job exit status after kill: %s' % exitStatus)

    def test_result(self):
        raise Exception(
            'JobTest is an abstract class. test_result must be implemented in subclass')
...
class LocalSubmission(JobsTest):
    ...
    def test_result(self):
        ...
        (correct, msg) = checkFiles(
            self.outputFiles[2:5], JobsTest.jobExamples.job1stdouterrModels)
        self.failUnless(correct, msg)
...
class SubmissionWithTransfer(JobsTest):
    ...
    def test_result(self):
        ...
        JobsTest.wf_ctrl.retrieve_job_stdouterr(
            self.myJobs[0],
            client_stdout,
            client_stderr)
        ...
        (correct, msg) = checkFiles(
            self.clientFiles[2:5], JobsTest.jobExamples.job1stdouterrModels)
        self.failUnless(correct, msg)
...
class JobPipelineWithTransfer(JobsTest):
    ...
    def test_result(self):
        ...
        models = JobsTest.jobExamples.job1OutputFileModels + JobsTest.jobExamples.job2OutputFileModels + \
        ...
        models = JobsTest.jobExamples.job1stdouterrModels + JobsTest.jobExamples.job2stdouterrModels + \
...
class ExceptionJobTest(JobsTest):
    ...
    def test_result(self):
        ...
        JobsTest.wf_ctrl.retrieve_job_stdouterr(
            jobid, client_stdout, client_stderr)
        self.clientFiles.append(client_stdout)
        self.clientFiles.append(client_stderr)

        (identical, msg) = checkFiles(self.clientFiles,
    ...
    def test_result(self):
        ...
        while line:
            ...
            if splitted_line[0] == "Grettings":
                self.failUnless(
                    line.rstrip() == "Grettings from process %d!" % (
                        process_num),
...
if __name__ == '__main__':
    ...
    if not job_examples_dir or not output_dir:
        raise RuntimeError(
            'The environment variables SOMA_WORKFLOW_EXAMPLES and SOMA_WORKFLOW_EXAMPLES_OUT must be set.')
    ...
    if line == "all":
        ...
        for job_type_index in selected_job_type_indexes:
            ...
            sys.stdout.write(
                "  => " + repr(job_types[int(job_type_index)]) + "\n")
